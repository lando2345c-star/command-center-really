...
function FBConnect({ onConnect, onSkip }) {
  ...
  const handleConnect = () => {
    ...
    onConnect({ id: "102907851890377", token: token.trim(), name: "Thumb Forecast", followers: 13000 });
  };
  return (
...
Use their actual visit data when giving advice. Be concise, specific, and encouraging. Reference real businesses they visited. Short paragraphs.`;
  ...
  facebook: {
    ...
    buildPrompt: (d, _, fb) => `You are a Facebook brand growth specialist for the Thumb Forecast page — a meteorology/weather brand by a Central Michigan University student with ${fb?.followers || "13K"} followers.
...
const INIT = {
  ...
  facebook: { reach: 1840, reachGoal: 5000, engagement: 4.2, engGoal: 8, posts: 3, postsGoal: 5, history: [1200, 1500, 1100, 1800, 2100, 1840],
    ...
    const fetchFB = async () => {
      ...
      try {
        ...
        if (json.data) {
          ...
          setD(prev => ({ ...prev, facebook: { ...prev.facebook, reach, engagement: engRate, history: hist.length >= 6 ? hist : prev.facebook.history } }));
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const fetchFB = async () => {
      setFbLoading(true);
      try {
        const fbUrl = `https://graph.facebook.com/v19.0/${fbCreds.id}/insights?metric=page_impressions_unique,page_post_engagements,page_fans&period=day&access_token=${fbCreds.token}`;
        const r = await fetch(`https://corsproxy.io/?${encodeURIComponent(fbUrl)}`);
        const json = await r.json();
        if (json.data) {
          const imp = json.data.find(m => m.name === "page_impressions_unique");
          const eng = json.data.find(m => m.name === "page_post_engagements");
  ...
  if (showFBConnect) return <FBConnect onConnect={creds => { setFbCreds(creds); setShowFBConnect(false); showToast(`✅ Connected: ${creds.name}!`); }} onSkip={() => setShowFBConnect(false)} />;
